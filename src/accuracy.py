import cv2
import tags
import sys
import os
sys.path.insert(0, '/home/pi/Git/apriltag/python')
import apriltag
import time

tags_expected = 75
nBots = 50
total_frames = 0
good_frames = 0
time_end = time.time() + 60
while (time.time() < time_end):
	frame = tags.get_frame()
	results = tags.find_tags(frame)
	ids = tags.extract_ids(results)
	cv2.imshow("live", frame)
	if cv2.waitKey(1) & 0xFF == ord('1'):
                break
	#print ("IDs found: ")
	#for n in ids:
	#	print (str(n))
	#print ("\n# corners: ")
	frame = tags.mark_bots(results, nBots, frame)
	cv2.putText(frame, str(int(time_end - time.time())), (25, 25), cv2.FONT_HERSHEY_SIMPLEX, 1, (100, 100, 255), 1)
	(frame, isGood) = tags.boundary(results, nBots, frame)
	if (len(ids) == tags_expected):
		good_frames = good_frames + 1
	elif (len(ids) > tags_expected):
		print ("WARNING: More Tags Detected Than Expected")
	total_frames = total_frames + 1
	print ("percent good: " + str(float(good_frames / total_frames)*100))
	cv2.imshow("post", frame)
print ("\n\nFinal Results\nPercent Good: " + str(float(good_frames / total_frames)*100))
print ("Total Frames: " + str(total_frames))
print ("Number Good: " + str(good_frames))
tags.end_prgm()
